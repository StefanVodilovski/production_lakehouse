...
async def fetch_data_from_api(current_page_url:str, params:dict, category:str, page:int, client:httpx.AsyncClient):
    ...
    if response.status_code ==429:
        raise Exception(f"Rate limit exceeded when fetching jobs for category {category} on page {page}. message: {response.text}")
    if response.status_code != 200:
        logger.warning(f"Failed to fetch jobs for category {category} on page {page}. Status code: {response.status_code}")
        return None
    data = response.json()
    return data

async def __handle_pagination_by_category(db: AsyncSession, search_endpoint: str, params: dict, category_id:int):
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        while True:
            ...
            if config.IS_TEST_MODE:
                ...
                data = await fetch_data_from_api(current_page_url, params, category, i, client)
                if data is None:
                    break
        ...
        try:
            ...
            for category in categories:
                ...
                if category_id is None:
                    logger.warning(f"Category ID not found for tag {category}. Using UNKNOWN_JOB_CATEGORY_ID_VALUE.")
                    category_id = config.UNKNOWN_JOB_CATEGORY_ID_VALUE
                await __handle_pagination_by_category(db, search_endpoint, params, category_id)
